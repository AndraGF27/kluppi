...
- **Hero phone start.** Bumped the `≤479px` `.header-image-list` / `.image-list-right` `padding-top` from `85vh` → `100vh`. At 85vh the collage still overlapped the tall phone hero content on short viewports; 100vh clears it and the images drift up into view on scroll. `≤991px` (`80vh`) left as-is (already correct at 768). Desktop untouched.
